...
class ModelicaSystem:
    ...
    def simulate(self, resultfile: Optional[str] = None, simflags: Optional[str] = None,
        ...
        om_cmd = ModelicaSystemCmd(runpath=self.tempdir, modelname=self._modelName, timeout=timeout)

        if resultfile is None:
            # default result file generated by OM
            self._resultfile = self.tempdir / f"{self._modelName}_res.mat"
        elif os.path.exists(resultfile):
            self._resultfile = pathlib.Path(resultfile)
        else:
            self._resultfile = self.tempdir / resultfile
        ...
        overrideFile = self.tempdir / f"{self._modelName}_override.txt"
    ...
    def createCSVData(self) -> pathlib.Path:
        ...
        csvFile = self.tempdir / f'{self._modelName}.csv'
    ...
    def linearize(self, lintime: Optional[float] = None, simflags: Optional[str] = None,
        ...
        overrideLinearFile = self.tempdir / f'{self._modelName}_override_linear.txt'
